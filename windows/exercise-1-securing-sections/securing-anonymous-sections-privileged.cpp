...
int main() {
    ...
    // step 3: get the low privileged process pid from user input
    DWORD dwPid;
    std::cout << "Enter the PID of the low privileged process: ";
    std::cin >> dwPid;

    // step 4: open a handle to the low privileged process
    ...
    // step 5: duplicate a read-only handle to a less privileged process
    ...
    std::cout << "Handle duplicated successfully!" << hTargetHandle << std::endl;

    std::cout << "Enter any key to read data from the section..." << std::endl;
    std::cin.get(); // wait for user input before reading data back
    std::cin.get();
    // step 6: read data back from the section to verify it's correct
    char buffer2[256];
    memcpy(buffer2, pMapView, sizeof(buffer2));
    std::cout << "Data read from the section: " << buffer2 << std::endl;
